...
int main() {
    ...
    auto imprimir = [&](const std::string& nombre, const std::vector<std::vector<int>>& res){
        std::cout << "\n=== RESULTADOS " << nombre << " ===\n";
        for (int i = 0; i < n; ++i) {
            if (!res[i].empty()) {
    ...
    imprimir("Boyer-Moore", bm_res);
    imprimir("Knuth-Morris-Pratt", kmp_res);
    imprimir("Rabin-Karp", rk_res);
